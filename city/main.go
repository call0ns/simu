...
func (P *city) getMap() {
	...
	mapFile, err := os.Open(P.mapFileName)
	if mapFile != nil {
		defer mapFile.Close()
	}
	if err != nil {
...
func (P *city) pickCity() string {
	...
	for k, _ := range P.storedParcel {
		...
		fmt.Println(k, sum, sumNeighbor)
		sum += sumNeighbor / 2
		fmt.Println("Add Nb", k, sum, sumNeighbor)
		if sum > maxVal {
			maxVal = sum
			maxcity = k
		}
...
func main() {
	...
	getPara.GetIntWithDefault(&P.timeFactor, "time", 60, "")
	getPara.GetStringWithDefault(&P.mapFileName, "map", "map.data", "")
	getPara.Finish()
